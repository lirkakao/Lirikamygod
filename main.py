...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "— Ответьте (Reply) на пересланное сообщение — ответ уйдёт от "
        "моего имени туда же.\n"
        "— <code>/post chat_id текст</code> — напишу в указанный чат "
        "от своего имени.\n\n"
...
async def cmd_post(message: Message):
    ...
    if len(parts) < 3:
        await message.answer("Использование: /post chat_id текст")
        return
    try:
        chat_id = int(parts[1])
    except ValueError:
...
async def forward_incoming(message: Message):
    if message.from_user and is_admin(message.from_user.id):
        return  # сообщения самих админов сюда не попадают (см. handle_admin_reply ниже)
...
async def handle_admin_reply(message: Message):
    ...
    target_chat_id = forward_map.get(key)

    if target_chat_id is None:
        await message.answer("⚠️ Отвечай Reply именно на пересланное сообщение бота.")
        return
    ...
    try:
        ...
        logging.exception("Не удалось отправить ответ: %s", e)
        await message.answer(f"Ошибка: {e}")
