...
# (admin_id, message_id в чате админа) -> (chat_id, message_id исходного сообщения)
forward_map: dict[tuple[int, int], tuple[int, int]] = {}
# admin_id -> chat_id последнего чата (запасной вариант без Reply — без привязки к сообщению)
...
async def deliver_content(message: Message, target_chat_id: int, override_text: str | None, reply_to: int | None = None):
    try:
        if message.photo:
            caption = override_text if override_text else message.caption
            await bot.send_photo(target_chat_id, message.photo[-1].file_id, caption=caption, parse_mode=None, reply_to_message_id=reply_to)
        elif message.video:
            caption = override_text if override_text else message.caption
            await bot.send_video(target_chat_id, message.video.file_id, caption=caption, parse_mode=None, reply_to_message_id=reply_to)
        elif message.voice:
            await bot.send_voice(target_chat_id, message.voice.file_id, reply_to_message_id=reply_to)
        elif message.video_note:
            await bot.send_video_note(target_chat_id, message.video_note.file_id, reply_to_message_id=reply_to)
        elif message.document:
            caption = override_text if override_text else message.caption
            await bot.send_document(target_chat_id, message.document.file_id, caption=caption, parse_mode=None, reply_to_message_id=reply_to)
        elif message.sticker:
            await bot.send_sticker(target_chat_id, message.sticker.file_id, reply_to_message_id=reply_to)
        elif override_text:
            await bot.send_message(target_chat_id, override_text, parse_mode=None, reply_to_message_id=reply_to)
        elif message.text:
            await bot.send_message(target_chat_id, message.text, parse_mode=None, reply_to_message_id=reply_to)
        else:
...
async def do_forward(message: Message):
    ...
    for admin_id in ADMINS:
        try:
            ...
            forward_map[(admin_id, sent.message_id)] = (message.chat.id, message.message_id)
            info = await bot.send_message(
                admin_id,
                f"↑ от {author} (id {uid}). Ответьте Reply на сообщение выше "
                f"(или просто напишите мне — уйдёт сюда же).",
            )
            forward_map[(admin_id, info.message_id)] = (message.chat.id, message.message_id)
...
@dp.message(
    ...
    F.func(lambda m: not (m.from_user and is_admin(m.from_user.id))),
)
...
async def handle_admin_message(message: Message):
    ...
    if raw.lower().startswith("/reply"):
        if not message.reply_to_message:
            ...
            return
        origin = forward_map.get((admin_id, message.reply_to_message.message_id))
        if origin is None:
            await message.answer("⚠️ Не нашёл, куда отправлять — отвечай Reply именно на свежее пересланное сообщение.")
            return
        target_chat_id, origin_message_id = origin
        override_text = raw[len("/reply"):].strip() or None
        await deliver_content(message, target_chat_id, override_text, reply_to=origin_message_id)
        return
    ...
    if message.reply_to_message:
        origin = forward_map.get((admin_id, message.reply_to_message.message_id))
        if origin:
            target_chat_id, reply_to = origin
    ...
    await deliver_content(message, target_chat_id, None, reply_to=reply_to)
